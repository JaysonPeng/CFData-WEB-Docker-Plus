...
本 Plus 版本使用独立的 GHCR 镜像，不覆盖原来的 CFData-WEB 镜像：

```text
ghcr.io/jaysonpeng/cfdata-web-plus:latest
```

Docker Compose 默认将配置和运行数据持久化到 `./data:/data`。
...
Web 版“非标优选”新增了 Cloudflare 地区 IP 数据源，可按地区筛选并生成 IP / IP:PORT / 地区标签格式，然后直接导入非标优选进行 TCPing、HTTPing 和测速。

该功能的数据来源及地区筛选思路来自：
- https://github.com/alienwaregf/Cloudflare-Country-Specific-IP-Filter
- 数据源：https://zip.cm.edu.kg/all.txt

请遵守上游项目的使用说明及当地法律法规。该功能仅用于个人学习、测试和研究用途。
