...
func handleWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	handlers := map[string]func(json.RawMessage){
		...
		"country_ip_regions": func(data json.RawMessage) {
			ctx, cancel := context.WithTimeout(r.Context(), 25*time.Second)
			defer cancel()
			items, err := fetchCountryIPSource(ctx)
			if err != nil {
				session.sendWSMessage("country_ip_regions_result", map[string]interface{}{"success": false, "message": err.Error()})
				return
			}
			regions := countryIPRegionStats(items)
			session.sendWSMessage("country_ip_regions_result", map[string]interface{}{"success": true, "regions": regions, "sourceURL": countryIPSourceURL})
		},
		"country_ip_fetch": func(data json.RawMessage) {
			var params countryIPFetchRequest
			if err := json.Unmarshal(data, &params); err != nil {
				session.sendWSMessage("country_ip_fetch_result", map[string]interface{}{"success": false, "message": "地区 IP 参数解析失败: " + err.Error()})
				return
			}
			if len(params.Regions) == 0 {
				session.sendWSMessage("country_ip_fetch_result", map[string]interface{}{"success": false, "message": "请至少选择一个地区"})
				return
			}
			ctx, cancel := context.WithTimeout(r.Context(), 25*time.Second)
			defer cancel()
			items, err := fetchCountryIPSource(ctx)
			if err != nil {
				session.sendWSMessage("country_ip_fetch_result", map[string]interface{}{"success": false, "message": err.Error()})
				return
			}
			result := buildCountryIPFetchResult(params, items)
			session.sendWSMessage("country_ip_fetch_result", result)
		},
